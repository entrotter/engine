"""Causal decisions and replay; custom Python providers require trusted native execution."""
...
def validate_decision(request: dict, raw: dict) -> dict:
    try:
        ...
        if (
            set(raw) != {"request_id", "choice", "reason"}
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            or re.fullmatch(r"0x(?:[0-9a-fA-F]{2}){0,4096}", result) is None
        ):
            raise AgentError("Agent preflight result must be hex bytes, maximum 4 KiB")
        preflight: dict = {"status": "success", "return_data": result}
    except RPCRejected:
    ...
    request = {
        ...
        "observation": {
            "step": step,
            "local_block_number": int(head["number"], 16),
            "local_block_hash": head["hash"],
            "timestamp": int(head["timestamp"], 16),
            "actor": actor,
            "local_chain_id": int(rpc.call("eth_chainId", []), 16),
            "native_balance_wei": str(
                int(rpc.call("eth_getBalance", [actor, "latest"]), 16)
            ),
            "tokens": [
                {**t, "balance_raw": balances[t["address"].lower()]} for t in tokens
            ],
            "completed_actions": [
                {"step": x["step"], "status": x["status"], "gas_used": x["gas_used"]}
                for x in previous
            ],
        },
        ...
        "preflight": preflight,
        "limits": {
            "remaining_requested_gas": gas_remaining,
            "max_response_bytes": MAX_RESPONSE_BYTES,
        },
        "choices": {
            "execute": "Execute exactly this allowlisted proposal on the owned local node",
...
class RiskPolicy:
    ...
    def decide(self, request: dict) -> dict:
        execute = (
            request["preflight"]["status"] == "success"
            and request["proposed_action"].get("gas", 21000)
            <= request["limits"]["remaining_requested_gas"]
        )
        return {
            "request_id": request["request_id"],
            "choice": "execute" if execute else "hold",
            "reason": "Current-state preflight and requested gas fit the policy"
            if execute
            else "Current-state preflight or requested gas exceeds the policy",
        }
...
class ReplayPolicy:
    def __init__(self, recording: dict):
        try:
            keys(
                recording, {"agent_version", "provider", "exchanges"}, "agent recording"
            )
            if (
                len(canonical(recording)) > MAX_RECORDING_BYTES
                or recording.get("agent_version") != AGENT_VERSION
                or not isinstance(recording.get("provider"), dict)
                or not isinstance(recording.get("exchanges"), list)
                or not 1 <= len(recording["exchanges"]) <= 32
            ):
            ...
            for exchange in recording["exchanges"]:
                ...
                if (
                    ...
                    or len(canonical(request)) > MAX_REQUEST_BYTES + 128
                ):
                    raise ValueError("Invalid observation")
                if request.get("request_id") != digest(
...
class AgentController:
    ...
    def start(self):
        if self.started:
            raise AgentError(
                "Controller is single-use; create a fresh controller for replay"
            )
        self.started = True
    ...
    def recording(self):
        ...
        recording = {
            "agent_version": AGENT_VERSION,
            "provider": deepcopy(self.provider.metadata),
            "exchanges": deepcopy(self.exchanges),
        }
        ReplayPolicy(
            recording
        )  # Enforce the same bounds on generated and imported records.
        return recording


def validate_selection(scenario: dict, controller: AgentController) -> None:
    """Check every selected proposal before launching an owned node."""
