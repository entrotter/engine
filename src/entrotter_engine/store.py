...
class ArtifactStore:
    ...
    def _locked(self):
        import fcntl
        descriptor = os.open(self.root / '.store.lock', os.O_CREAT | os.O_RDWR | os.O_NOFOLLOW | os.O_NONBLOCK, 0o600)
        try:
        ...
        with os.scandir(self.root) as entries:
            for entry in entries:
                ...
                if not stat.S_ISREG(info.st_mode) or info.st_nlink != 1:
                    raise OSError('Artifact directory must contain only regular, singly linked files')
    ...
    def put(self, report):
        ...
        with self._locked():
            ...
            if total + len(raw) > self.max_bytes or count + 1 > self.max_files:
                raise StoreFull('Artifact store quota exceeded; export or remove reports locally')
            descriptor, name = tempfile.mkstemp(prefix='.report-', suffix='.tmp', dir=self.root)
            try:
