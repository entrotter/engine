...
def approve(token: str, spender: str, amount: int) -> dict:
    return {"to": address(token, "token"), "data": "0x095ea7b3" +
            address_word(spender) + uint_word(amount), "gas": 100000}


def wrap_native(weth: str, amount: int) -> dict:
    uint_word(amount)
    return {"to": address(weth, "wrapped native token"), "data": "0xd0e30db0",
            "value_wei": str(amount), "gas": 100000}


def exact_input_single(*, router: str, token_in: str, token_out: str, fee: int,
                       recipient: str, deadline: int, amount_in: int,
                       minimum_out: int, sqrt_price_limit_x96: int = 0) -> dict:
    ...
    fields = [address_word(token_in), address_word(token_out), uint_word(fee, 24),
              address_word(recipient), uint_word(deadline), uint_word(amount_in),
              uint_word(minimum_out), uint_word(sqrt_price_limit_x96, 160)]
    return {"to": address(router, "router"), "data": "0x414bf389" + "".join(fields),
            "gas": 500000}
...
def token_balances(rpc, tokens: list[dict], actor: str) -> dict[str, str]:
    return {token["address"].lower(): str(read_uint(rpc, token["address"],
            "0x70a08231" + address_word(actor))) for token in tokens}
