...
def number(
    value: Any, name: str, *, minimum: str = "0", maximum: str = "1e30"
) -> Decimal:
    if (
...
def policy(p: Any) -> dict:
    ...
    if p.get("type") not in {"hold", "circuit_breaker"}:
        raise ValidationError(
            "Only built-in hold and circuit_breaker policies are supported"
        )
    if p["type"] == "circuit_breaker":
        number(
            p.get("drawdown_trigger"), "drawdown_trigger", minimum="0.001", maximum="1"
        )
...
def validate(raw: Any) -> dict:
    ...
    if not isinstance(raw.get("id"), str) or not re.fullmatch(
        r"[a-z0-9][a-z0-9-]{0,63}", raw["id"]
    ):
        raise ValidationError(
            "id must contain lowercase letters, digits and hyphens (1-64)"
        )
    ...
    if (
        not isinstance(prov.get("description"), str)
        or not 1 <= len(prov["description"]) <= 2000
    ):
    ...
    if mode == "fixture":
        ...
        if prov.get("kind") != "synthetic":
            raise ValidationError(
                "v0.1 fixture paths must be explicitly labelled synthetic"
            )
        m = keys(
            raw.get("market"),
            {
        ...
        if (
            number(m["initial_cash"], "initial_cash") == 0
            and number(m["initial_units"], "initial_units") == 0
        ):
        ...
        if not isinstance(m.get("symbol"), str) or not re.fullmatch(
            r"[A-Z0-9_-]{1,12}", m["symbol"]
        ):
            raise ValidationError(
                "symbol must be 1-12 uppercase alphanumeric characters"
            )
        policy(raw.get("baseline"))
        policy(raw.get("candidate"))
        ...
        if prov.get("kind") != (
            "local-evm" if mode == "evm-local" else "historical-fork"
        ):
        ...
        for token in tokens:
            ...
            if not isinstance(token.get("symbol"), str) or not re.fullmatch(
                r"[A-Z0-9_-]{1,12}", token["symbol"]
            ):
                raise ValidationError(
                    "Token symbol must be 1-12 uppercase alphanumeric characters"
                )
        steps = raw.get("steps")
        ...
        for slot in steps:
            ...
            if set(slot) != {"baseline", "candidate"}:
                raise ValidationError(
                    "Every slot needs baseline and candidate (null = no-op)"
                )
            for action in slot.values():
                ...
                if not isinstance(data, str) or not re.fullmatch(
                    r"0x(?:[0-9a-fA-F]{2}){0,16384}", data
                ):
                    raise ValidationError(
                        "data must be even-length hex (maximum 16 KiB)"
                    )
        ...
        if mode == "evm-fork":
            source = keys(
                raw.get("source"), {"chain_id", "block_number", "block_hash"}, "source"
            )
            integer(source.get("chain_id"), "source.chain_id", 1, 2**53 - 1)
            integer(source.get("block_number"), "source.block_number", 1, 2**53 - 1)
            if "block_hash" in source and (
                not isinstance(source["block_hash"], str)
                or not re.fullmatch(r"0x[0-9a-fA-F]{64}", source["block_hash"])
            ):
