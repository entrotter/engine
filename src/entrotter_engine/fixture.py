...
def build_policy(spec: dict) -> Policy:
    return Hold() if spec["type"] == "hold" else CircuitBreaker(spec["drawdown_trigger"])
...
def trial(market: dict, spec: dict) -> dict:
    ...
    for step, raw_price in enumerate(market["prices"]):
        ...
        rows.append({"step": step, "observed_price": text(price), "action": action,
                     "cash": text(cash), "units": text(units), "equity": text(equity),
                     "execution_price": text(execution_price) if execution_price is not None else None,
                     "fee": text(fee)})
    initial = D(market["initial_cash"]) + D(market["initial_units"]) * D(market["prices"][0])
    final = D(rows[-1]["equity"])
    return {"policy": spec, "trace": rows,
            "metrics": {"initial_equity": text(initial), "final_equity": text(final),
                        "return_pct": text((final-initial)/initial*100),
                        "max_drawdown_pct": text(max_drawdown*100),
                        "fees": text(fees), "trades": trades}}
...
def run_fixture(scenario: dict) -> dict:
    with localcontext() as ctx:
        ...
        delta = D(candidate["metrics"]["final_equity"]) - D(baseline["metrics"]["final_equity"])
        return seal({"schema_version": VERSION, "engine_version": VERSION,
                     "mode": "fixture", "scenario": scenario,
                     "baseline": baseline, "candidate": candidate,
                     "comparison": {"final_equity_delta": text(delta)},
                     "assumptions": ["Synthetic exogenous price path; not a historical replay.",
